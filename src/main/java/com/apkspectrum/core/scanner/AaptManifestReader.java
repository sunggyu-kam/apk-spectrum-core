...
public class AaptManifestReader {
    ...
    public AaptManifestReader(AaptXmlTreePath manifestPath, ManifestInfo targetManifest) {
        ...
        manifestInfo = targetManifest != null ? targetManifest : new ManifestInfo();
    ...
    public void readBasicInfo() {
        ...
        if (tagNode != null) {
            manifestInfo.usesSdk.targetSdkVersion =
                    getAttrIntegerValue(tagNode, "targetSdkVersion");
        ...
        manifestInfo.compatibleScreens =
                getCompatibleScreens(manifestPath.getNodeList("/manifest/compatible-screens"));
        manifestInfo.supportsScreens =
                getSupportsScreens(manifestPath.getNodeList("/manifest/supports-screens"));
        manifestInfo.supportsGlTexture =
                getSupportsGlTexture(manifestPath.getNodeList("/manifest/supports-gl-texture"));
        manifestInfo.usesConfiguration =
                getUsesConfiguration(manifestPath.getNodeList("/manifest/uses-configuration"));
        manifestInfo.usesFeature =
                getUsesFeature(manifestPath.getNodeList("/manifest/uses-feature"));
        manifestInfo.usesLibrary =
                getUsesLibrary(manifestPath.getNodeList("/manifest/uses-library"));

        // display to launchur
        AaptXmlTreeNode[] launchers =
        ...
        if (launchers != null && launchers.length > 0) {
            ...
            launchers = manifestPath.getNodeList(
                    "/manifest/application/activity-alias/intent-filter/" + "category[@" + namespace
                            + "name='android.intent.category.LAUNCHER']/../" + "action[@"
                            + namespace + "name='android.intent.action.MAIN']");
            if (launchers != null && launchers.length > 0) {
                manifestInfo.featureFlags |= ManifestInfo.MANIFEST_FEATURE_LAUNCHUR;
            }
        }
        ...
        // startup
        if (manifestPath.getNode("/manifest/uses-permission[@" + namespace
                + "name='android.permission.RECEIVE_BOOT_COMPLETED']") != null) {
    ...
    public void readPermissions() {
        ...
        if (permTag != null && permTag.length > 0) {
            ...
            manifestInfo.usesPermission =
                    usesPermissionList.toArray(new UsesPermissionInfo[usesPermissionList.size()]);
        ...
        if (permTag != null && permTag.length > 0) {
            ...
            manifestInfo.usesPermissionSdk23 =
                    usesPermissionList.toArray(new UsesPermissionSdk23Info[0]);
        }
        ...
        if (permTag != null && permTag.length > 0) {
            ...
            manifestInfo.permission =
                    permissionList.toArray(new PermissionInfo[permissionList.size()]);
        }

        Log.i("read permission-tree");
        permTag = manifestPath.getNodeList("/manifest/permission-tree");
        if (permTag != null && permTag.length > 0) {
            List<PermissionTreeInfo> permissionList = new ArrayList<>();
            for (int idx = 0; idx < permTag.length; idx++) {
            ...
            manifestInfo.permissionTree =
                    permissionList.toArray(new PermissionTreeInfo[permissionList.size()]);
        }
    }

    public WidgetInfo[] getWidgetList(String apkFilePath) {
        List<WidgetInfo> widgetList = new ArrayList<>();
        ...
        for (AaptXmlTreeNode node : widgetTag) {
            WidgetInfo[] shorcuts =
                    getLuncherShortCuts(apkFilePath, getAttrValue(node, "resource"));
            if (shorcuts != null) {
        ...
        widgetTag = SystemUtil.joinArray(
                manifestPath.getNodeList(
                        "/manifest/application/activity/intent-filter/" + "action[@" + namespace
    ...
    private void readWidgetResourceInfo(String apkFilePath, WidgetInfo widget,
            ResourceInfo[] resources) {
        if (resources == null || resources.length <= 0 || apkFilePath == null
    ...
    private WidgetInfo[] getLuncherShortCuts(String apkFilePath, String shortcutResId) {
        ...
        for (ResourceInfo r : shortcutRes) {
            ...
            resourceMap.put(r.name + "/shortcuts",
                    new ResourceEntry(r.name + "/shortcuts", shortcutIds));
            ...
            for (int i = 0; i < shortcuts.length; i++) {
                ...
                resSet = getAttrResourceEntry(shortcuts[i], "shortcutDisabledMessage",
                        widgetNamespace);
                ...
                AaptXmlTreeNode[] intents = shortcuts[i].getNodeList("intent");
                if (intents != null) {
                    for (AaptXmlTreeNode intent : intents) {
        ...
        Log.v("getLuncherShortCuts() "
                + (xmlRes == null ? "null" : xmlRes.configuration + ", " + xmlRes.name));

        return widgets;
    }

    private int getV(String config) {
        if (config == null || !config.matches(".*v(\\d+).*")) {
            return 0;
        }
    ...
    private WidgetInfo getActivityShortCut(AaptXmlTreeNode node) {
        ...
        widget.resourceMap.put("label", resSet);
        widget.labels = resSet.getValue();
        if (widget.labels == null) widget.labels = manifestInfo.application.labels;
    ...
    private MetaDataInfo[] getMetaDataInfo(AaptXmlTreeNode[] metaDataNodeList) {
        if (metaDataNodeList == null || metaDataNodeList.length == 0) {
            return null;
        }
    ...
    private DataInfo[] getDataInfo(AaptXmlTreeNode[] dataNodeList) {
        if (dataNodeList == null || dataNodeList.length == 0) {
            return null;
        }
    ...
    private IntentFilterInfo[] getIntentFilterInfo(AaptXmlTreeNode[] intentFilterNodeList) {
        if (intentFilterNodeList == null || intentFilterNodeList.length == 0) {
            return null;
        }

        List<IntentFilterInfo> intentList = new ArrayList<>();
    ...
    private Integer checkIntentFlag(IntentFilterInfo[] intentFilterInfo) {
        ...
        for (IntentFilterInfo intent : intentFilterInfo) {
            if (intent.ation != null && (featureFlag & (ApkInfo.APP_FEATURE_MAIN
                    | ApkInfo.APP_FEATURE_STARTUP)) != (ApkInfo.APP_FEATURE_MAIN
                            | ApkInfo.APP_FEATURE_STARTUP)) {
            ...
            if (intent.category != null && (featureFlag & (ApkInfo.APP_FEATURE_LAUNCHER
                    | ApkInfo.APP_FEATURE_DEFAULT)) != (ApkInfo.APP_FEATURE_LAUNCHER
                            | ApkInfo.APP_FEATURE_DEFAULT)) {
    ...
    public void readActivityAliasInfo() {
        AaptXmlTreeNode[] activityTag =
                manifestPath.getNodeList("/manifest/application/activity-alias");
        if (activityTag == null || activityTag.length == 0) {
            return;
        }

        List<ActivityAliasInfo> list = new ArrayList<>();
        for (int idx = 0; idx < activityTag.length; idx++) {
    ...
    private CompatibleScreensInfo[] getCompatibleScreens(AaptXmlTreeNode[] nodeList) {
        ...
        List<CompatibleScreensInfo> intentList = new ArrayList<>();
        for (AaptXmlTreeNode node : nodeList) {
            ...
            if (screenNode != null) {
                List<CompatibleScreensInfo.Screen> screensList = new ArrayList<>();
    ...
    private SupportsScreensInfo[] getSupportsScreens(AaptXmlTreeNode[] tagNodeList) {
        if (tagNodeList == null || tagNodeList.length == 0) {
            return null;
        }

        List<SupportsScreensInfo> list = new ArrayList<>();
    ...
    private UsesConfigurationInfo[] getUsesConfiguration(AaptXmlTreeNode[] tagNodeList) {
        ...
        List<UsesConfigurationInfo> list = new ArrayList<>();
        for (AaptXmlTreeNode n : tagNodeList) {
    ...
    private UsesFeatureInfo[] getUsesFeature(AaptXmlTreeNode[] tagNodeList) {
        ...
        List<UsesFeatureInfo> list = new ArrayList<>();
        for (AaptXmlTreeNode n : tagNodeList) {
    ...
    private UsesLibraryInfo[] getUsesLibrary(AaptXmlTreeNode[] tagNodeList) {
        ...
        List<UsesLibraryInfo> list = new ArrayList<>();
        for (AaptXmlTreeNode n : tagNodeList) {
    ...
    private SupportsGlTextureInfo[] getSupportsGlTexture(AaptXmlTreeNode[] tagNodeList) {
        ...
        List<SupportsGlTextureInfo> list = new ArrayList<>();
        for (AaptXmlTreeNode n : tagNodeList) {
    ...
    public ResourceInfo[] getResourceValues(String id) {
        ...
        while (id != null && id.startsWith("@") && !id.matches("@0x0+\\s*")) {
            ...
            if (resVal == null || resVal.length == 0 || id == resVal[0].name) break;
            id = resVal[0].name;
        }

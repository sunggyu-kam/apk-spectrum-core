...
public class SignatureReport {
        ...
        try {
            ...
            for (int i = 0; i < signatures.length; i++) {
                certificates[i] = (X509Certificate) cf.generateCertificate(
                        new ByteArrayInputStream(signatures[i].toByteArray()));
            }
    ...
    public SignatureReport(JarFile jf) throws Exception {
        ...
        List<X509Certificate> timestampList = new ArrayList<>();
            ...
            if (signers != null) {
                for (CodeSigner signer: signers) {
                    if (!ss.contains(signer)) {
                        ss.add(signer);
                        for (Certificate cert: signer.getSignerCertPath()
                                                     .getCertificates()) {
                            certs.add((X509Certificate) cert);
                        }
                        Timestamp ts = signer.getTimestamp();
                        if (ts != null) {
                            for (Certificate cert: ts.getSignerCertPath()
                                                     .getCertificates()) {
                ...
                if (entryName.startsWith("META-INF/")){
                    if (entryName.toUpperCase().endsWith(".RSA")
                            || entryName.toUpperCase().endsWith(".DSA")
                            || entryName.toUpperCase().endsWith(".EC")) {
                        try {
                            //certificates = new PKCS7(jf.getInputStream(je))
                            //                .getCertificates();
                            //certificates = new X509Certificate[1];
        ...
        if (!timestampList.isEmpty()) {
            timestamp = timestampList.toArray(
                    new X509Certificate[timestampList.size()]);
        }
    }
...
/*
    private static void printExtensions(String title, CertificateExtensions exts
            , PrintStream out) throws Exception {
        int extnum = 0;
        Iterator<Extension> i1 = exts.getAllExtensions().iterator();
        Iterator<Extension> i2 = exts.getUnparseableExtensions().values()
                                .iterator();
    ...
    private static void printHexDump(byte[] dump, OutputStream out) {
        ...
        if (instance == null) {
            Log.e("No such any encoder");
            return;
        }

        try {
            clazz.getMethod("encodeBuffer", byte[].class, OutputStream.class)
                 .invoke(instance, dump, out);
...
*/
    ...
    private void dumpCert(Certificate cert, PrintStream out)
        ...
        if (rfc) {
            ...
            out.println(Base64.getMimeEncoder().encodeToString(
                        cert.getEncoded()));
    ...
    private static String getCertFingerPrint(String mdAlg, Certificate cert)
        ...
        return Util.toHexString(digest, ':', true);
    }

...
public class AdbVersionManager implements Comparator<String> {
    ...
    public static final String CACHE_FILE_PATH =
            FileUtil.getTempPath() + File.separator + "adb_list_cache.txt";
    ...
    public static boolean checkAdbVersion(File adbfile) {
        ...
        if (version.compareTo(MIN_ADB_VERSION) > 0) {
            ...
            String message = String.format(
                    "Required minimum version of adb: %1$s." + "Current version is %2$s : %3$s",
                    MIN_ADB_VERSION, version, adbfile.getAbsolutePath());
    ...
    public static void loadDefaultAdbs() {
        ...
        getAdbVersion(_RFile.BIN_ADB.get(), true);
    }
        ...
        synchronized (cacheMap) {
            if (jsonCache == null) {
                ...
                while (iterator.hasNext()) {
                    ...
                    String md5 = tmp[0];
                    if (!md5.isEmpty()
                            && md5.equals(FileUtil.getMessageDigest(new File(path), "MD5"))) {
    ...
    public static void saveCache() {
        synchronized (cacheMap) {
            ...
            String transMultiLine = jsonCache.toJSONString().replaceAll("^\\{(.*)\\}$", "{\n$1\n}")
                    .replaceAll("(\"[^\"]*\":(\"[^\"]*\")?([^\",]*)?,)", "$1\n");
            // .replaceAll("(\"[^\"]*\":(\"[^\"]*\")?([^\",\\[]*(\\[[^\\]]\\])?)?,)", "$1\n");

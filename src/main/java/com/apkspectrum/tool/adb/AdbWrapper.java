...
public class AdbWrapper {
    ...
    public static String getAdbCmd() {
        ...
        if (cmd == null) {
            ...
            if (_RProp.B.ADB_POLICY_SHARED.get()) {
                String[] runProcess = null;
                int waitCnt = 0;
                do {
                    if (runProcess != null) {
                        if (waitCnt++ > 5) {
                            Log.d("waiting for running adb daemon only one. but any daemon be not exit. "
                                    + runProcess.length);
                            break;
                        } ;
    ...
    static public boolean pullApk(final String device, final String srcApkPath,
            final String destApkPath, final ConsoleOutputObserver listener) {
            ...
            public boolean ConsolOutput(String output) {
                if (output.trim().endsWith(": Permission denied")) {
                    ...
                    String[] mkdir = {"mkdir", "-p",
                            tmpPath + realApkPath.substring(0, realApkPath.lastIndexOf("/"))};
    ...
    static public boolean pull(String device, String srcApkPath, String destApkPath,
            ConsoleOutputObserver listener) {
        if (adbCmd == null) return false;
        String[] param;
        if (device == null || device.isEmpty()) {
        ...
        /*
         * || (!result[result.length-1].endsWith("s)") &&
    ...
    static public boolean push(String device, String srcApkPath, String destApkPath,
            ConsoleOutputObserver listener) {
        if (adbCmd == null) return false;
        String[] param;
        if (device == null || device.isEmpty()) {
    ...
    static public String[] installOnSdcard(String device, String apkPath,
            ConsoleOutputObserver listener) {
        if (adbCmd == null) return null;
        String[] param;
        if (device == null || device.isEmpty()) {
    ...
    static public String[] uninstall(String device, String packageName,
            ConsoleOutputObserver listener) {
        if (adbCmd == null) return null;
        String[] param;
        if (device == null || device.isEmpty()) {

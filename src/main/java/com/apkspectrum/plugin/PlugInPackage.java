...
public class PlugInPackage {
    ...
    public PlugInGroup[] getTopPlugInGroup() {
        List<PlugInGroup> list = new ArrayList<>();
        for (PlugInGroup g : pluginGroups) {
    ...
    private PlugIn[] createPlugInInstance(File pluginFile, Manifest manifest) {
        if (manifest == null) return null;
        Log.v(manifest.packageName + " : " + fingerprint + ", ver " + manifest.versionCode + ", "
            ...
            switch (c.type) {
                ...
                default:
                    ...
                    try {
                        ...
                        plugin = (PlugIn) clazz.getConstructor(PlugInPackage.class, Component.class)
                                .newInstance(this, c);
    ...
    private Map<String, String> loadConfiguration(Manifest manifest) {
    ...
    private Map<String, Map<String, String>> loadResource(Manifest manifest) {
    ...
    public Map<String, Object> getChangedProperties() {
        ...
        Map<String, String> cfg = getConfigurations();
        if (manifest.configuration != null) {
            for (Configuration c : manifest.configuration) {
